...
def strike_at_index(collection: list, strike_index: int, radius: int):
    ...
    if is_index_valid(collection, left_strike_index) and is_index_valid(collection, right_strike_index) \
            and is_index_valid(collection, strike_index):
...
number_str_list = [str(num) for num in numbers]
