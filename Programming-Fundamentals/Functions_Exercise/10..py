...
def exchange_list_at_index(collection: list, index: int):
    ...
    if not is_index_valid(collection, index):
        print("Invalid index")
...
def parse_collection_to_integer(collection: list):
    parsed_collection = []
...
numbers = parse_collection_to_integer(elements)


command = input()
while command != "end":
    cmd_arguments = command.split()
    cmd_type = cmd_arguments[0]

    if cmd_type == "exchange":
        index = int(cmd_arguments[1])

        numbers = exchange_list_at_index(numbers, index)

    elif cmd_type == "max":
        cmd_filter = cmd_arguments[1]
        ...
        count = int(cmd_arguments[1])
        cmd_filter = cmd_arguments[2]
